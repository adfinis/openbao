	...
	if _, ok := barrier.(logical.Transaction); ok {
		...
		writeTable = func(mt *MountTable, prefix string) (int, error) {
			...
			if mount != "" && !found {
				...
				// we're removing this auth mount, so the live in-memory table
				// still has the removed entry and its namespace context.
				targetEntry, err := c.auth.find(ctx, func(me *MountEntry) bool { return me.UUID == mount })
				if err != nil {
					return -1, fmt.Errorf("failed to resolve deleted auth mount %q: %w", mount, err)
				}
				if targetEntry == nil || targetEntry.Namespace() == nil {
					return -1, fmt.Errorf("requested removal of auth mount %q but its namespace context is unavailable", mount)
				}

				view := NamespaceView(barrier, targetEntry.Namespace())
				path := path.Join(prefix, mount)
				if err := view.Delete(ctx, path); err != nil {
					return -1, fmt.Errorf("requested removal of auth mount from namespace %v but failed: %w", targetEntry.Namespace().ID, err)
				}
			}
