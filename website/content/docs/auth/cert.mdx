...
1. Configure it with trusted certificates that are allowed to authenticate:
  ...
  For client certificates that use a URI SAN as their identity, configure
  `allowed_uri_sans` and set `alias_name_source` to `uri_san`:

  ```shell
  $ bao write auth/cert/certs/spiffe-workload \
    display_name=spiffe-workload \
    certificate=@ca.pem \
    allowed_uri_sans=spiffe://example.org/ns/default/sa/web \
    alias_name_source=uri_san
   ```
