...
- `alias_name_source` `(string: "common_name")` - The source used to derive the
  identity alias name during login. Valid values are `common_name`, which uses
  the client certificate Common Name, and `uri_san`, which uses a URI SAN matched
  by `allowed_uri_sans`. Configure `allowed_uri_sans` narrowly enough to select
