...
| Clustered HA fragmented reconnect, default fanout | `indexed-repair-20260602T142940Z` | Pass with expected client transients | 300s, 64 workers, secondary #1 outage beyond journal horizon with default `reconcile_max_range_drilldown_rpcs=32`. Primary `journal_range_too_old_total` increased by 4; primary range-digest requests increased by 39,301 and fetch requests by 3,974, with zero range/fetch rejections or scan failures. Secondary #1 returned to `streaming` with `lag_entries=0`, reconcile count increased by 3, indexed repair total reached 4, 3,974 indexed repair ranges, zero local KID fallback scans, zero full-bucket fallback, zero indexed proof mismatches, zero load failures, and strict checkpoint verification passed on both secondaries with 1,024/1,024 matched ranges and `physical_scan_used=false`. |
| Clustered HA fragmented fanout cap | `fragmented-fanout-20260602T142058Z` | Pass with expected client transients | 300s, 64 workers, secondary #1 outage beyond journal horizon with `reconcile_max_range_drilldown_rpcs=1` only on secondary #1. Primary `journal_range_too_old_total` increased by 4; primary range-digest requests increased by 3,877 instead of 39,301 in the same-binary default-cap run. Secondary #1 used 3,865 coarse fetches covering 6,411 ranges, returned to `streaming` with `lag_entries=0`, indexed repair total 4, zero local KID fallback scans, zero load failures, zero full-bucket fallback, zero indexed proof mismatches, zero scan failures, and strict checkpoint verification passed on both secondaries with 1,024/1,024 matched ranges and `physical_scan_used=false`. |
...
- The latest paired clustered fragmented-reconnect smokes exercised the
  indexed-repair path under the same 300s/64-worker outage shape on the same
  rebuilt binary. The default-cap run stayed scan-free but drove 39,301 primary
  range-digest requests; the low-cap run stayed scan-free and reduced that
  primary fanout to 3,877 by switching to coarser proof-backed fetch spans.
