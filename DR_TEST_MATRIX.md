...
| S11 | Dynamic tuning under HA load | `scripts/dr_local_test.sh --topology ha tuning-load-smoke` | Update primary and secondary DR tuning while mixed load is running, then force HA handoffs | Tuning writes succeed, new active nodes report the updated profile, workload exits cleanly, both secondaries return to `streaming` with lag 0, and terminal verification passes through the supported verification path |
...
| S22 | DR pre-seed manifest and bundle validation | `go test ./vault -run 'TestDRPreSeed|TestDRRelationshipManager.*PreSeed' -count=1` | Generate, export, accept, or import seed material with relationship, checkpoint, algorithm, lineage, expiry, integrity, KID/VID, artifact-format, segment descriptor, and local-only scrub mismatches | Valid seed material is accepted only for the matching fresh relationship; stale lineage, wrong primary cluster, wrong relationship, wrong range/checksum version, missing scrub metadata, expired seed material, tampered bundle bytes, invalid segment metadata, and local-only seed entries are rejected before trusting restored storage |
