...
| S15 | HA indexed-repair validation | `scripts/dr_local_test.sh --topology ha indexed-repair-smoke` or `make dr-test-ha-indexed-repair-smoke` | Exercise indexed-bucket repair under sustained HA handoff pressure and prove reconciliation does not fall back to full local scan | `flat_accumulator_indexed_repair_total`, `flat_accumulator_indexed_repair_ranges_total`, `local_kid_index_bucket_loads_total`, and `local_kid_index_entries_loaded_total` increase; `scan_failures_total`, `local_kid_index_fallback_scans_total`, full-bucket fallback, proof mismatches, and local KID-index load failures do not increase; both secondaries return to `streaming` with `lag_entries=0` and `reconcile_phase=idle`; sentinel convergence succeeds, no status events are dropped, and exhaustive verification passes on primary and both secondaries |
