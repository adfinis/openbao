...
| A31 | DR pre-seed manifest, bundle export/import, and accept/apply lifecycle | `go test ./vault -run 'TestDRPreSeed|TestDRRelationshipManager.*PreSeed|TestSystemBackend_DRSpecialPaths|TestSystemBackend_DRSensitiveFields' -count=1` | Pass; pre-seed manifests are relationship-bound, signed by the primary DR transport CA, reject missing/tampered/wrong-CA provenance, stale lineage, relationship mismatch, algorithm mismatch, missing local-only scrub metadata, expired material, and invalid segmented artifact metadata; bundle validation rejects integrity drift, local-only paths, and tampered segment descriptors; primary export uses checkpoint artifacts and excludes local-only paths; segmented export plans produce per-segment descriptors and checkpoint-bound segment payloads; secondary segmented import durably stages segments across manager restart, rejects incomplete or tampered staging, requires explicit replacement confirmation, replaces replicated storage while preserving cluster-local paths, and applies the checkpoint baseline only when secondary mode is enabled or restored with the same relationship material |
| A32 | DR transport CA rotation lifecycle | `go test ./vault -run 'TestDRRelationshipManager_(PrimaryTransportCARotationLifecycle|SecondaryAcceptsPrimaryTransportCATrustBundleLifecycle|EnableSecondaryPersistsPrimaryCATrustSet)|TestDRSystemBackend_TransportCARotationEndpoints|TestDRClusterClient_VerifyCASignedCertFromStagedTrustAnchor|TestHeartbeatResponse_ActiveClusterCertRequiresConfiguredCA' -count=1` plus `scripts/dr_local_test.sh --topology ha transport-ca-rotation-smoke`, `scripts/dr_local_test.sh --topology ha transport-ca-rotation-load-smoke`, and `scripts/dr_local_test.sh --topology ha transport-ca-rotation-chain-smoke` | Pass; primary stage/read/activate/retire state is unit-gated, activation tokens include staged trust anchors, secondary accept verifies signed trust bundles before updating local trust, active and staged CA-signed primary leaves verify during overlap, `SyncKeyring` unwrap binds to the CA that verified the primary leaf, stale cached leaves are rejected after CA retirement, unknown/tampered/stale CA material is rejected, heartbeat-advertised leaves require a configured CA, and the clustered HA smokes validate stage/activate/retire across primary and secondary active handoff, mixed load, repeated rotation chains, stale bundle rejection, primary API verification, and strict-secondary checkpoint verification |
...
make dr-test-ha-transport-ca-rotation-smoke
make dr-test-ha-transport-ca-rotation-load-smoke
make dr-test-ha-transport-ca-rotation-chain-smoke
...
| E17 | Clustered reconcile budget pressure | From a clean streaming HA topology, run `scripts/dr_local_test.sh --topology ha reconcile-budget-smoke`. | Secondary #1 is stopped beyond the journal horizon and restarted with an intentionally tiny reconcile RPC byte budget. It must report `budget_exhausted` with phase/reason/RPC byte accounting, remain incomplete instead of falsely advancing a successful repair, and expose the failure in DR status. Primary pressure counters must move without unexpected rejection counters. Primary API verification and secondary #2 checkpoint verification must pass; secondary #1 verification is intentionally skipped because non-convergence is the expected fail-closed behavior. |
| E18 | DR transport CA rotation under HA | From a streaming HA topology, run `scripts/dr_local_test.sh --topology ha transport-ca-rotation-smoke` or `make dr-test-ha-transport-ca-rotation-smoke`. | The primary can stage, activate, and retire DR transport CAs; both secondaries accept each signed trust bundle at the right phase; stale activated/staged bundles are rejected after retirement; primary and secondary active handoff restore DR transport connectivity; both secondaries return to `streaming` and pass checkpoint verification. |
| E19 | DR transport CA rotation under HA load | From a streaming HA topology, run `scripts/dr_local_test.sh --topology ha transport-ca-rotation-load-smoke` or `make dr-test-ha-transport-ca-rotation-load-smoke`. | The staged/activated/retired transport CA lifecycle succeeds while mixed read/write/status load and primary stepdowns are active; stale bundles are rejected after retirement; final primary API verification and strict-secondary checkpoint verification pass. PUT/GET failures during handoff windows are availability signals unless terminal verification shows missing or mismatched data. |
| E20 | DR transport CA repeated rotation chain | Run `scripts/dr_local_test.sh --topology ha transport-ca-rotation-chain-smoke --rotations 3` or `make dr-test-ha-transport-ca-rotation-chain-smoke`; add `--no-reset` only when intentionally reusing an already-streaming HA topology. | Multiple stage/activate/retire cycles succeed on the same relationship without resetting between rotations; stale bundles from the current and prior generations are rejected on both secondaries; final strict-secondary checkpoint verification passes. |
